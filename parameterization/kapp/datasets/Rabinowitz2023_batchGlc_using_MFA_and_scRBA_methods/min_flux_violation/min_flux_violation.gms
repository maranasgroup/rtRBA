...
put 'rxn', 'v_exp_lb', 'v_sm', '% diff from v_exp_lb'/;
loop(sm_j,
    if ( (s_v_exp_lb.l(sm_j) gt %vmin%) or (s_v_exp_ub.l(sm_j) gt %vmin%),
        put sm_j.tl:0, v_exp_lb(sm_j):0:15, (v_sm.l(sm_j)/%nscale%):0:15, (100*((v_sm.l(sm_j)/%nscale%)-v_exp_lb(sm_j))/v_exp_lb(sm_j)):0:2/;
    );
);
putclose;
